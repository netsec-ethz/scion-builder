...
Version: devel
Release: 1
Summary: SCION Path Server
URL: https://www.scion-architecture.net
License: Apache License, v2.0

Source0: bazel-out/k8-fastbuild/bin/scion-path-server.tar.gz
...
%build
tar xvf bazel-out/k8-fastbuild/bin/scion-path-server.tar.gz
...
install -m 755 ./usr/bin/path_srv %{buildroot}%{_bindir}/path_srv

#mkdir -p %{buildroot}%{_unitdir}/
mkdir -p %{buildroot}/lib/systemd/system/
cp ./lib/systemd/system/scion-path-server@.service %{buildroot}/lib/systemd/system/scion-path-server@.service
...
%files
%attr(0755, root, root) %{_bindir}/path_srv
%attr(0644, root, root) /lib/systemd/system/scion-path-server@.service
